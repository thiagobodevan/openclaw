...
Inspect shows identity, load status, source, manifest capabilities, policy flags, diagnostics, install metadata, bundle capabilities, and any detected MCP or LSP server support without importing plugin runtime by default. JSON output includes the plugin manifest contracts, such as `contracts.agentToolResultMiddleware`, `contracts.trustedToolPolicies`, and `contracts.finalToolInputPolicies`, so operators can audit trusted-surface declarations before enabling or restarting a plugin. Final tool input policies are manifest-first: bundled and installed plugins must declare every policy id; installed plugins must also be explicitly enabled. Add `--runtime` to load the plugin module and include registered hooks, tools, commands, services, gateway methods, and HTTP routes. Runtime inspection reports a plugin load diagnostic when an active plugin fails to register a declared final tool input policy. Every activating runtime load rejects that candidate registry instead of running without the policy; Gateway startup and reload inherit the same fail-closed behavior. Installs and repairs stay in `openclaw plugins install`, `openclaw plugins update`, and `openclaw doctor --fix`.

`plugins.entries.<id>.requiredFinalToolInputPolicies` is operator-owned desired state. An activating load rejects a missing, disabled, denied, undeclared, or unregistered required policy. Use `inspect --json` to verify the manifest declaration and `inspect --runtime --json` to inspect runtime load diagnostics when the plugin package is present. The requirement does not automatically install, enable, or allowlist a plugin.
